...
public class Witch extends Entity implements Updateble, Renderable {
    ...
    public static boolean enter = false, esc = false, event = false, tryEvent = false, activeOne = false, end = false;
    ...
    public Witch(int x, int y, int width, int height, BufferedImage sprite) {
        ...
        esqueletoDialogue[1].add("Minha despensa já estava ficando vazia,");
    ...
    public void update() {
        ...
        if (Game.player.getX() >= 528 && Game.player.getX() <= 608 && Game.player.getY() >= 128) {
            ...
            if (anotacaoDialogue.isSinalizeExit() && tryEvent) {
                event = false;
                esc = false;
                enter = false;
